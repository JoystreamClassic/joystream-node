...
class TorrentPlugin extends EventEmitter {
  ...
  */
  ...
  isStarted () {
    return this.status.session.state === SESSION_STATE.started
  }

  isModeSet () {
    return this.status.session.mode !== SESSION_MODE.not_set
  }

  toSellMode (sellerTerms, callback) {
    this.plugin.to_sell_mode(this.infoHash, sellerTerms, (err) => {
      callback(err)
    })
  }

  toBuyMode (buyerTerms, callback) {
    this.plugin.to_buy_mode(this.infoHash, buyerTerms, (err) => {
      callback(err)
    })
  }

  toObserveMode (callback) {
    this.plugin.to_observe_mode(this.infoHash, (err) => {
      callback(err)
    })
  }
