...
class TorrentPlugin extends EventEmitter {
  ...
  constructor (status, plugin, peers, infoHash) {
    ...
    // Regularly request status alerts
    setInterval(() => {
      this.plugin.post_torrent_plugin_status_updates(this.infoHash)
    }, 1000)
  ...
  to_sell_mode (sellerTerms, callback) {
    ...
    plugin.to_sell_mode(infoHash, sellerTerms, (err, result) => {
      if (!err) {
        ...
        plugin.start(infoHash, (err, result) => {
          if (!err) {

            // Regularly request status alerts from peer plugins on this torrent
            setInterval(() => {
              plugin.post_peer_plugin_status_updates(infoHash)
            }, 1000)
