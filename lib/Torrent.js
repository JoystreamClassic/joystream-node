...
class Torrent extends EventEmitter {
  ...
  constructor (handle, plugin) {
    ...
    setInterval(this._refreshJoyStreamPeerList.bind(this), cleanAnnouncedJSPeersMapInterval)
  }
   ...
   * * * * * * * * * * * * */
   ...
   _refreshJoyStreamPeerList () {
     // Clean announces JS peers map in order to remove old peers.
     for (var [secondaryInfoHash, timestamp] of this.announcedJSPeersAtTimestamp.entries()) {
       let now = Date.now()

       if (now - timestamp > outOfDatePeerTime) {
         this.announcedJSPeersAtTimestamp.delete(secondaryInfoHash)
       }
     }
   }
