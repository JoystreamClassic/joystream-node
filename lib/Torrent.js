'use strict'

var sha1 = require('sha1')
const EventEmitter = require('events')

var debug = require('debug')('torrent')
var _ = require('lodash')

//const cleanAnnouncedJSPeersMapInterval = 60 * 60 * 1000 // 1h
// const outOfDatePeerTime = 60 * 60 * 1000 // 1h

class Torrent extends EventEmitter {

  constructor (handle, plugin) {
    super()

    this.handle = handle
    this.plugin = plugin // joystream extension (libtorrent session plugin)
    this.announcedJSPeersAtTimestamp = new Map()

    this.infoHash = handle.infoHash()
    this.secondaryInfoHash = this._secondaryInfoHash(this.infoHash)

    // store last known statuses of torrent plugin and peers
    this._pluginStatus = {}
    this._peerPluginStatuses = []
  }

  /* * * * * * * * * * * * *
   *  Secondary Info Hash
   * * * * * * * * * * * * */

  addJSPeerAtTimestamp (address, timestamp) {
    this.announcedJSPeersAtTimestamp.set(address, timestamp)
  }

  refreshJoyStreamPeerList (outOfDatePeerTime) {
    // Clean announces JS peers map in order to remove old peers.
    for (var [address, timestamp] of this.announcedJSPeersAtTimestamp.entries()) {
      let now = Date.now()

      if (now - timestamp > outOfDatePeerTime) {
        this.announcedJSPeersAtTimestamp.delete(address)
      }
    }
  }

  // Review needed !
  _secondaryInfoHash (infoHash) {
    var newInfoHash = infoHash + '_JS'
    return sha1(newInfoHash)
  }

  _onDhtGetPeersReply (peers) {
    var timestamp = Date.now()

    for (var i in peers) {
      this.addJSPeerAtTimestamp(peers[i].address, timestamp)
      this.handle.connectPeer(peers[i])
    }
  }

  // Torrent status
  status () {
    return this.handle.status()
  }

  // Return a copy of the last known torrent plugin status (protocol session and connection statuses)
  pluginStatus () {
    return _.clone(this._pluginStatus)
    // TODO: return a helper class that wraps the status and provides helper methods
    // to "compute" information about the status
    //return new TorrentPluginStatus(_pluginStatus)
  }

  // Return an array of copies of last known peer plugin statuses
  peerStatuses () {
    return _.clone(this._peerPluginStatuses)
    // return Array of PeerPluginStatus elements
  }

  addJSPeerAtTimestamp (address, timestamp) {
    this.announcedJSPeersAtTimestamp.set(address, timestamp)
  }

  _onPeerPluginStatusUpdate (statuses) {
    this._peerPluginStatuses = _.clone(statuses)
    this.emit('peerPluginStatusUpdates', statuses)
  }

  _onResumeData (buff) {
    this.emit('resumedata', buff)
  }

  _onResumeDataFailed () {
    this.emit('error', new Error('resume data generation failed'))
  }

  _onMetaData (torrentInfo) {
    this.emit('metadata', torrentInfo)
  }

  _onRemoved () {
    // TODO: have the torrent cleanup and emit a signal that is was removed
    // set it's handle to null because the handle
    // doesn't point to an existing torrent anymore
  }

  _onFinished () {
    this.emit('finished')
  }

  _onStatusUpdate (status) {
    this.emit('status_update', status)
  }

  _onStateChanged () {
    this.emit('state_changed')
  }

  _onResumed () {
    this.emit('resumed')
  }

  _onPaused () {
    this.emit('paused')
  }

  _onTorrentPluginStatusUpdate (status) {
    this._pluginStatus = _.clone(status)
    this.emit('pluginStatusUpdate', status)
    //this.emit('pluginStatusUpdate', new TorrentPluginStatus(status))
  }

  _onConnectionAdded (endpoint, status) {
    const addr = endpoint.address + ':' + endpoint.key
    this.emit('connectionAdded', addr, status)
    //this.emit('connectionAdded', new ConnectionStatus(status))
  }

  _onConnectionRemoved (endpoint) {
    const addr = endpoint.address + ':' + endpoint.key
    this.emit('connectionRemoved', addr)
  }

  // Torrent Plugin Controls

  toSellMode (sellerTerms, callback = () => {}) {
    this.plugin.to_sell_mode(this.infoHash, sellerTerms, callback)
  }

  toBuyMode (buyerTerms, callback = () => {}) {
    this.plugin.to_buy_mode(this.infoHash, buyerTerms, callback)
  }

  toObserveMode (callback = () => {}) {
    this.plugin.to_observe_mode(this.infoHash, callback)
  }

  setLibtorrentInteraction (mode, callback = () => {}) {
    this.plugin.set_libtorrent_interaction(this.infoHash, mode, callback)
  }

  stopPlugin (callback = () => {}) {
    this.plugin.stop(this.infoHash, callback)
  }

  startPlugin (callback = () => {}) {
    this.plugin.start(this.infoHash, callback)
  }

  pausePlugin (callback = () => {}) {
    this.plugin.pause(this.infoHash, callback)
  }

  startUploading (connection, contractSk, finalPkHash, callback = () => {}) {
    const endPoint = connection.endPoint
    const buyerTerms = connection.announcedModeAndTermsFromPeer.buyer.terms
    this.plugin.start_uploading(this.infoHash, endPoint, buyerTerms, contractSk, finalPkHash, callback)
  }

  startDownloading (contract, downloadInfoMap, callback = () => {}) {
    this.plugin.start_downloading(this.infoHash, contract, downloadInfoMap, callback)
  }

}

module.exports = Torrent
