...
const dhtAnnounceInterval = 15 * 60 * 1000
...
class Session extends EventEmitter {
  ...
  constructor ({port}) {
    ...
    setInterval(() => {
      // Announce secondy info hash to DHT
  ...
  */
  ...
  _peerConnectAlert (alert) {
    ...
    if (this.torrents.has(infoHash)) {
      const torrent = this.torrents.get(infoHash)
      torrent._onPeerConnected(alert.ip)
    }
  }
  ...
  _peerDisconnectedAlert (alert) {
    ...
    if (this.torrents.has(infoHash)) {
      const torrent = this.torrents.get(infoHash)
      torrent._onPeerDisconnected(alert.ip)
    }
  }
  ...
  _peerPluginStatusUpdateAlert (alert) {
    ...
    for (var status of statuses) {
      torrent._onPeerPluginStatusUpdate(status)
    }
  }
  ...
  _torrentPluginAdded (alert) {
    ...
    if (torrent) {
      torrent._onTorrentPluginAdded(alert.status)
    }
  }
  ...
  _torrentPluginRemoved (alert) {
    ...
    if (torrent) {
      torrent._onTorrentPluginRemoved()
    }
  }
  ...
  _peerPluginAdded (alert) {
    ...
    if (torrent) {
      torrent._onPeerPluginAdded(alert.ip, alert.status)
    }
  }
  ...
  _peerPluginRemoved (alert) {
    ...
    if (torrent) {
      torrent._onPeerPluginRemoved(alert.ip)
    }
  }
