  ...
  */
  ...
  _metadataReceivedAlert (alert) {
    ...
    if (torrentInfo && torrent) {
      ...
      this.emit('metadata_received', torrentInfo)
    }
  }
  ...
  _addTorrentAlert (alert) {
    ...
    if (alert.error) {
      alertDebug('Error adding torrent:' + alert.error.message)
      return
    }
  ...
  _saveResumeDataAlert (alert) {
    ...
    if (torrent) {
      torrent.setResumeDataGenerationResult(alert.resumeData)
      this.emit('resumedata', infoHash, alert.resumeData)
    } else {
      alertDebug('Torrent not found !')
    }
  }
  ...
  _saveResumeDataFailedAlert (alert) {
    ...
    if (torrent) {
      ...
      torrent.setResumeDataGenerationResult()
    } else {
      alertDebug('Torrent not found !')
    }
  }
