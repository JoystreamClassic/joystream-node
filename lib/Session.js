  ...
  */
  ...
  _stateUpdateAlert (alert) {
    ...
    for (var i in status) {
      ...
      if (torrent) {
        torrent.onStatusUpdate(status[i])
      }
    }
  }
  ...
  _peerConnectAlert (alert) {
    ...
    if (this.torrents.has(infoHash)) {
      const torrent = this.torrents.get(infoHash)
      torrent.onPeerConnected(alert.ip)
    }
  }
  ...
  _peerDisconnectedAlert (alert) {
    ...
    if (this.torrents.has(infoHash)) {
      const torrent = this.torrents.get(infoHash)
      torrent.onPeerDisconnected(alert.ip)
    }
  }
  ...
  _torrentPluginStatusUpdateAlert (alert) {
    for (var status of alert.statuses) {
      ...
      if (torrent) {
        torrent.onTorrentPluginStatusUpdate(status)
      }
    }
  }
  ...
  _peerPluginStatusUpdateAlert (alert) {
    ...
    for (var status of statuses) {
      torrent.onPeerPluginStatusUpdate(status.endPoint, status)
    }
  }
  ...
  _peerPluginAdded (alert) {
    ...
    if (torrent) {
      torrent.onPeerPluginAdded(alert.ip, alert.status)
    }
  }
  ...
  _peerPluginRemoved (alert) {
    ...
    if (torrent) {
      torrent.onPeerPluginRemoved(alert.ip)
    }
  }
  ...
  _connectionAddedToSession (alert) {
    ...
    if (torrent) {
      torrent.onConnection(alert.ip, alert.status)
    }
  }
  ...
  _connectionRemovedFromSession (alert) {
    ...
    if (torrent) {
      torrent.onConnectionRemoved(alert.ip, alert.status)
    }
  }
