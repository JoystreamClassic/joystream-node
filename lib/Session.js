...
const DHTAnnounceInterval = 900000 // 15mins
...
class Session extends EventEmitter {
  ...
  constructor ({port, secondaryDHT = true}) {
    ...
    if (secondaryDHT) {
      setInterval(this.dhtForSecondaryHash.bind(this), DHTAnnounceInterval)
    }
   ...
   * for a specific info hash.
   */
  dhtForSecondaryHash () {
    for (var [secondaryInfoHash] of this.torrentsBySecondaryHash.entries()) {
      this.dhtAnnounce(secondaryInfoHash)
      this.dhtGetPeers(secondaryInfoHash)
    }
  }
