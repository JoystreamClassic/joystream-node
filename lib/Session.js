...
class Session extends EventEmitter {
  ...
  */
  ...
  _addTorrentAlert (alert) {
    ...
    // Instead we emit a differnet named alert which indicates that the torrent was not added
    // through the joystream extension request
  ...
  _connectionAddedToSession (alert) {
    ...
    if (torrent) {
      torrent._onConnectionAdded(aler.pid, alert.status)
    }
  }
