...
const statusUpdateInterval = 100 // 100ms
...
class Session extends EventEmitter {
  ...
  constructor ({port, assistedPeerDiscovery = true}) {
    super()
    ...
    // Process alerts and request peer and plugins status updates at regular interval
    // NOTE: alert processing and status update inerval shouldn't necessarily be the same?
    setInterval(() => {
      // Pop alerts
  ...
  */
  ...
  dhtAnnounce (infoHash) {
    // NOTE: We are announcing the local listening port. This may not be the
    // same as publicly mapped port if we are behind NAT!
